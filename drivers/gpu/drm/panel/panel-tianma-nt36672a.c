...
static int tianma_panel_backlight_update_status(struct backlight_device *bl)
{

	return 0;
}

static int tianma_panel_backlight_get_brightness(struct backlight_device *bl)
{
...
static int tianma_panel_backlight_init(struct panel_info *pinfo)
{
	 ...
	 */
	if (!bl->wled) {
		pr_err("[tianma-nt36672a] backlight registration failed\n");
		rc = -EINVAL;
	} else {
		...
		if (rc) {
			pr_info("[tianma-nt36672a] backlight:");
...
pr_err("In nt36672a panel probe: mipi_dsi_attach returned: %d\n", err);
	return err;	
}
